...
- API キーはブラウザの localStorage に保存され、**Google の Gemini API 以外には送信されません**
- 動画は Gemini API に送られ Google 側で処理されます（Google の利用規約に従います）
- 本ソフトウェアは「現状のまま」提供され、生成結果の正確性は保証されません（[LICENSE](LICENSE) 参照）

## 謝辞

本プロジェクトの設計・実装・テストには [Claude Code](https://claude.com/claude-code)（Anthropic）を用い、各コミットに共著者（Co-authored-by）として記録しています。
